...
w2v_model.wv.most_similar(positive=["franco"])
...
dbscan = DBSCAN(metric="euclidean", eps=3, min_samples=100) # you can change these parameters, given just for example 
cluster_labels = dbscan.fit_predict(standard_embedding) # where X - is your matrix, where each row corresponds to one document (line) from the docs, you need to cluster 
