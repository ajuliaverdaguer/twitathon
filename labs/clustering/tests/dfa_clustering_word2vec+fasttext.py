...
model_word2vec = Word2Vec(min_count=20,
                          window=20,
                          size=100,
                          sample=6e-5, 
                          alpha=0.03, 
...
model_word2vec.build_vocab(tweets["sentence"], progress_per=10000)

# ## Train model

model_word2vec.train(tweets["sentence"], total_examples=model_word2vec.corpus_count, epochs=50, report_delay=1)
...
model_word2vec.wv.most_similar(positive=["pablo_iglesias"])
...
import fasttext
import os
def create_model_fasttext(texts):
    temp_file = "temp.txt"
...
model_fasttext = create_model_fasttext(tweets["text_clean"])

# ## Analysis

model_fasttext.get_nearest_neighbors("puigdemont")

# # Clustering

# ## Vectorize

vectors_word2vec = model_word2vec.wv.vectors
indices_word2vec = {word: model_word2vec.wv.vocab[word].index for word in model_word2vec.wv.vocab}


# +
def vectorize_word2vec(line):
    words = list()
    for word in line:
        try:
            w2v_idx = indices_word2vec[word]
        except KeyError:
            continue
        words.append(vectors_word2vec[w2v_idx])
    if words:
...
def vectorize_fasttext(line):
    words = list()
    for word in line:
        words.append(np.array(model_fasttext[word]))
    if words:
        words = np.asarray(words)
        return words.mean(axis=0)
    if not words:
...
tweets["vector_word2vec"] = tweets["sentence"].apply(vectorize_word2vec)
tweets.head()

tweets["vector_fasttext"] = tweets["sentence"].apply(vectorize_fasttext)
tweets.head()

tweets["is_valid_vector"] = tweets["vector_fasttext"].apply(lambda x: type(x) == np.ndarray)
tweets.head()

tweets_valid = tweets[tweets["is_valid_vector"] == True]
len(tweets_valid)

X = np.stack(tweets_valid["vector_fasttext"], axis=0)

# ## Clustering model

embedding_2d = umap.UMAP(random_state=31).fit_transform(X)

dbscan = DBSCAN(metric="euclidean", eps=1, min_samples=1000)
...
tweets_valid[tweets_valid["cluster"] == 3]
