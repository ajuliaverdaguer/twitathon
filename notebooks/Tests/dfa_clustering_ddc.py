...
w2v_model.wv.most_similar(positive=["pedro_sanchez", "podemos"], negative=["psoe"], topn=3)
...
dbscan = DBSCAN(metric="cosine", eps=0.07, min_samples=100) # you can change these parameters, given just for example 
...
for index, row in tweets_valid.iterrows():
    if "pablo_iglesias" in row["sentence"]:
        print(row)
